...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateSql = "UPDATE volunteer_form_submissions SET is_archived = TRUE WHERE volunteer_form_submission_id IN ($placeholders)";
    ...
    if ($stmt = $conn->prepare($updateSql)) {
        ...
        if ($stmt->execute()) {
            echo json_encode(["message" => "Records archived successfully."]);
        } else {
            http_response_code(500);
            echo json_encode(["message" => "Error archiving records."]);
        }
        $stmt->close();
    } else {
        http_response_code(500);
    ...
    echo json_encode(["message" => "Invalid request method. Use POST for archiving."]);
}
