...
for (let tabIndex = 0; tabIndex < folderTabs.length; tabIndex++) {
  folders[tabIndex].style.transition =
    "transform 0.5s, border-top 0.3s, background-color 0.1s";

  const originalColor = window.getComputedStyle(
    folderBodies[tabIndex]
  ).backgroundColor;
  folderTabs[tabIndex].addEventListener("mouseenter", () => {
    ...
    folderTabs[tabIndex].style.backgroundColor = brightenColor(
      originalColor,
      20
    );
    folderBodies[tabIndex].style.backgroundColor = brightenColor(
      originalColor,
      20
    );
  });
...
const btnVolunteerAboutContainer = document.getElementById(
  "btnVolunteerAboutContainer"
);
